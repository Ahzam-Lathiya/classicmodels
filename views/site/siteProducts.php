...
for($i=0; $i<count($allProducts); $i++)
{
  ...
  echo '<td>' . $i + 1 . '</td>';
  echo '<td>' . "<a href=products/product/" . $allProducts[$i]['productCode'] . " target='/blank'>" . $allProducts[$i]["productName"] . '</a>' . '</td>';
  ...
  echo "<td>" . '<input class="productCounter" type="number" min="0">' . "</td>";
  echo '<td id="identifier" hidden>' . $allProducts[$i]['productCode'] . "</td>";
...
function createRow(data, position)
{
  ...
  a.target = "/blank";
  a.textContent = data['productName'];
  ...
  */
  ...
  td = document.createElement('td');
  td.insertAdjacentHTML('beforeend', '<input class="productCounter" type="number" min="0">');
  
  tr.appendChild(td);


  td = document.createElement('td');
  td.setAttribute('hidden', null);
  td.id = "identifier";
  td.textContent = data['productCode'];
